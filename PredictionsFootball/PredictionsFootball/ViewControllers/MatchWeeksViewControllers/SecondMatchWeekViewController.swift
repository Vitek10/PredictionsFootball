...
class SecondMatchWeekViewController: UIViewController, UITextFieldDelegate {
  ...
  // First Match View
    ...
    @IBAction func firstPredictionsTapped(_ sender: Any) {
        let postFirstRef = Database.database().reference().child("secondWeek").child("firstMatchPredictions").child(userID!)
                         let postFirstObject = [
        ...
        @IBAction func secondPredictionsTapped(_ sender: Any) {
          let postSecondRef = Database.database().reference().child("secondWeek").child("secondMatchPredictions").child(userID!)
       ...
       @IBAction func thirdPredictionsTapped(_ sender: Any) {
          let postSecondRef = Database.database().reference().child("secondWeek").child("thirdMatchPredictions").child(userID!)
                 let postSecondObject = [
      ...
      @IBAction func fourthPredictionsTapped(_ sender: Any) {
          let postSecondRef = Database.database().reference().child("secondWeek").child("fourthMatchPredictions").child(userID!)
                 let postSecondObject = [
      ...
      @IBAction func fifthPredictionsTapped(_ sender: Any) {
          let postSecondRef = Database.database().reference().child("secondWeek").child("fifthMatchPredictions").child(userID!)
                 let postSecondObject = [
    ...
    func firstMatchData(){
        ref = Database.database().reference().child("secondWeek").child("firstMatch").child("match")
    ...
    func firstMatchPredictionData(matchHomeResult:String,matchGuestResult:String){
        ref = Database.database().reference().child("secondWeek").child("firstMatchPredictions").child(userID!)
    ...
    func secondMatchData(){
        ref = Database.database().reference().child("secondWeek").child("secondMatch").child("match")
        ref.observe(DataEventType.value, with: {(snapshot) in
            if snapshot.childrenCount>0 {
                self.matchesList.removeAll()
                for matches in snapshot.children.allObjects as![DataSnapshot]{
    ...
    func secondMatchPredictionData(matchHomeResult:String,matchGuestResult:String){
        ref = Database.database().reference().child("secondWeek").child("secondMatchPredictions").child(userID!)
        ref.observeSingleEvent(of: .value, with: {(snapshot) in
                self.matchesListPredictions.removeAll()
                    let matchObject = snapshot.value! as? [String : AnyObject]
                    let firstPredictionScore = matchObject?["team_1"]
    ...
    func secondMatchPredictionLabel(){
     ...
     func thirdMatchData(){
            ref = Database.database().reference().child("secondWeek").child("thirdMatch").child("match")
            ref.observe(DataEventType.value, with: {(snapshot) in
                if snapshot.childrenCount>0 {
                    self.matchesList.removeAll()
                    for matches in snapshot.children.allObjects as![DataSnapshot]{
        ...
        func thirdMatchPredictionData(matchHomeResult:String,matchGuestResult:String){
            ref = Database.database().reference().child("secondWeek").child("thirdMatchPredictions").child(userID!)
            ref.observeSingleEvent(of: .value, with: {(snapshot) in
                    self.matchesListPredictions.removeAll()
                        let matchObject = snapshot.value! as? [String : AnyObject]
                        let firstPredictionScore = matchObject?["team_1"]
    ...
    func fourthMatchData(){
            ref = Database.database().reference().child("secondWeek").child("fourthMatch").child("match")
            ref.observe(DataEventType.value, with: {(snapshot) in
                if snapshot.childrenCount>0 {
                    self.matchesList.removeAll()
                    for matches in snapshot.children.allObjects as![DataSnapshot]{
        ...
        func fourthMatchPredictionData(matchHomeResult:String,matchGuestResult:String){
            ref = Database.database().reference().child("secondWeek").child("fourthMatchPredictions").child(userID!)
            ref.observeSingleEvent(of: .value, with: {(snapshot) in
                    self.matchesListPredictions.removeAll()
                        let matchObject = snapshot.value! as? [String : AnyObject]
                        let firstPredictionScore = matchObject?["team_1"]
    ...
    func fifthMatchData(){
            ref = Database.database().reference().child("secondWeek").child("fifthMatch").child("match")
            ref.observe(DataEventType.value, with: {(snapshot) in
                if snapshot.childrenCount>0 {
                    self.matchesList.removeAll()
                    for matches in snapshot.children.allObjects as![DataSnapshot]{
        ...
        func fifthMatchPredictionData(matchHomeResult:String,matchGuestResult:String){
            ref = Database.database().reference().child("secondWeek").child("fifthMatchPredictions").child(userID!)
            ref.observeSingleEvent(of: .value, with: {(snapshot) in
                    self.matchesListPredictions.removeAll()
                        let matchObject = snapshot.value! as? [String : AnyObject]
                        let firstPredictionScore = matchObject?["team_1"]
    ...
    func highScore(){
        let postFirstRef = Database.database().reference().child("UserScore").child(userID!).child("Highscore_2")
                let postFirstObject = [
                    "Score": currentScore
                    ] as [String : Int]
                        postFirstRef.setValue(postFirstObject,withCompletionBlock: {error, ref in
